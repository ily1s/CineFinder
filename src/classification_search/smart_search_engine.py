...
class SmartSearchEngine:
    ...
    def build_index(self):
        ...
        for field in self.fields:
            if self.doc_lengths[field]:
                self.avg_doc_length[field] = np.mean(
                    list(self.doc_lengths[field].values())
                )
    ...
    def search(self, query, top_n=10):
        ...
        results = self.df.loc[
            result_indices, ["Title", "Overview", "Genres", "Director", "Release_Date"]
        ].copy()
    ...
    def save_index(self, folder_path="../data/index_data"):
        ...
        for field, terms_dict in self.inverted_index.items():
            ...
            for term, postings in terms_dict.items():
                serializable_index[field][term] = [
                    [int(doc_id), int(freq)] for doc_id, freq in postings
                ]
        ...
        metadata = {
            "N": self.N,
            "doc_lengths": {
                field: {int(k): v for k, v in lengths.items()}
        ...
        for field, terms_dict in serializable_index.items():
            for term, postings in terms_dict.items():
                self.inverted_index[field][term] = [
                    (doc_id, freq) for doc_id, freq in postings
                ]
